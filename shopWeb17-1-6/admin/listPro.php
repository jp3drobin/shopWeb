...
$where = $keywords ? "where p.pName like '%{$keywords}%'" : null; //数据库模糊查询
//得到数据库中所有商品
$sql = "SELECT p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.sPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName FROM shop_pro AS p JOIN shop_cate c ON p.cId=c.id {$where}  ";
...
$sql = "SELECT p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.sPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName FROM shop_pro AS p JOIN shop_cate c ON p.cId=c.id {$where} {$orderBy} limit {$offset},{$pageSize}";
$rows = fetchAll($sql);
?>
<!doctype html>
<html>
            ...
            <div class="text">
                ...
                <div class="bui_select">
                    <select id="" class="select" onchange="change(this.value)">
                        <option>-请选择-</option>
                        <option value="sPrice asc">由低到高</option>
                        <option value="sPrice desc">由高到底</option>
                    </select>
                </div>
            </div>
            <div class="text">
    ...
    <table class="table" cellspacing="0" cellpadding="0">
        ...
        <?php foreach ($rows as $row): ?>
            <tr>
                ...
                <td>
                    <input type="checkbox" id="c<?php echo $row['id']; ?>" class="check"
                ...
                <td><?php echo $row['sPrice']; ?>元</td>
                <td align="center">
                    <input type="button" value="详情" class="btn"
                           onclick="showDetail(<?php echo $row['id']; ?>,'<?php echo $row['pName']; ?>')">
                    ...
                    <div id="showDetail<?php echo $row['id']; ?>" style="display:none;">
                        <table class="table" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                                <td width="20%" align="right">商城价格</td>
