...
{
    $arr = $_POST;
    $arr['pubTime'] = time(); //商品添加时间
    $path = "./uploads"; //默认图片上传目录
    ...
    if ($res && $pid) {
        ...
        foreach ($uploadFiles as $uploadFile) {
            if (file_exists("../image_800/" . $uploadFile['name'])) {
                unlink("../image_800/" . $uploadFile['name']); //如果存在则删除
            }
...
{
    $sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.sPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from shop_pro as p join shop_cate c on p.cId=c.id";
    $rows = fetchAll($sql);
    return $rows;
}
...
{
    $sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.sPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName,p.cId from shop_pro as p join shop_cate c on p.cId=c.id where p.id={$id}";
    $row = fetchOne($sql);
    return $row;
}
...
function getAllPros()
{
    $sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.sPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName,p.cId from shop_pro as p join shop_cate c on p.cId=c.id ";
    $rows = fetchAll($sql);
    return $rows;
}
...
{
    $sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.sPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName,p.cId from shop_pro as p join shop_cate c on p.cId=c.id where p.cId={$cid} limit 4";
    $rows = fetchAll($sql);
    return $rows;
}
...
{
    $sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.sPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName,p.cId from shop_pro as p join shop_cate c on p.cId=c.id where p.cId={$cid} limit 4,4";
    $rows = fetchAll($sql);
    return $rows;
}
